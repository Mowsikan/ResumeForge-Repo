  ...
  if (!user) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-50 to-green-50 flex items-center justify-center">
        ...
        >
          <div
            className={`${isMobile ? "w-20 h-20" : "w-24 h-24"} bg-gradient-to-br from-green-500 to-blue-600 rounded-3xl flex items-center justify-center mx-auto mb-6`}
          >
            <Download
          ...
          >
            {filteredResumes.map((resume) => {
              const template = getTemplateById(
                resume.template_id || "modern-simple",
              );
              const isRecent =
                (new Date().getTime() -
                  ...
                  (1000 * 60 * 60) <
                24;
              const safeResumeData = ensureResumeDataCompatibility(
                resume.resume_data,
              );
              return (
                    ...
                    <CardContent className="p-0">
                      ...
                      >
                        ...
                        <div className="absolute top-2 right-2 flex flex-col gap-1">
                          ...
                          {isRecent && (
                            <Badge
                              className={`bg-blue-500 text-white shadow-sm ${isMobile ? "text-xs px-2 py-1" : "text-xs"}`}
                            >
        ...
        >
          ...
          >
            ...
            >
              <div className="flex items-center">
                <Download
                  className={`${isMobile ? "w-5 h-5" : "w-6 h-6"} mr-3 text-green-600`}
                />
                <span className="truncate">
                  {selectedResume?.title || "Downloaded Resume"}
                </span>
                <Badge
                  className={`${isMobile ? "ml-2" : "ml-3"} bg-green-100 text-green-700`}
                >
                  Downloaded
                </Badge>
              </div>
          ...
          >
            {selectedResume && (
              <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
                <ResumePreview
                  data={ensureResumeDataCompatibility(
